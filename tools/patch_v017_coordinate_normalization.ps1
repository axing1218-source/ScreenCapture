...
# v0.8.17
# Normalize Gemini translation boxes at the parser boundary. Gemini is instructed
# to return 0..1000 boxes but can occasionally return source-pixel coordinates.
# We infer the coordinate space from text/region geometry, not screenshot-size rules.
# Downstream renderers then always receive one canonical 0..1000 coordinate system.

# Source-level diagnostics, independent of WHLOG.dll.
...
# Include diagnostics after Windows headers.
if (-not $src.Contains('#include "WeShotDiag.h"')) {
    $src = $src.Replace('#include <winrt/Windows.Data.Json.h>', "#include <winrt/Windows.Data.Json.h>`r`n#include \"WeShotDiag.h\"")
}

# Add flexible translation-box reader immediately after the canonical readBox helper.
$anchor = @'
...
# v0.8.16 parser has source + sourceLines before the box is accepted. Use those
# values to infer coordinate space, then store only normalized coordinates.
$oldParse = @'
                TranslationBlock b;
...
# Make the model instruction unambiguous as a first defense; runtime inference
# remains the fallback when it still returns pixels.
$src = $src.Replace(
...
foreach ($needle in @('readTranslationBox', 'translate-box image=', 'coord=', 'WeShotDiag::append', 'NEVER use source pixel coordinates')) {
