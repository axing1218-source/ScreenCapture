...
namespace {
    bool shortcutDarkMode()
    {
        auto* setting = Setting::get();
        return setting && setting->getToolFlag(L"app", L"darkMode", false);
    }
    uint32_t shortcutText() { return shortcutDarkMode() ? 0xE8EAEDFF : 0x333333FF; }
    uint32_t shortcutSurface() { return shortcutDarkMode() ? 0x2B2C30FF : 0xFFFFFFFF; }
    uint32_t shortcutBorder() { return shortcutDarkMode() ? 0x4A4C52FF : 0xE0E0E0FF; }
    uint32_t shortcutHover() { return shortcutDarkMode() ? 0x383A3FFF : 0xFFFFFFFF; }
...
WinSettingShortcut::WinSettingShortcut(Ling::WinBase* parent):Ling::Node(parent)
{
    ...
    for (auto& key:keys)
    {
        ...
        btn->setColor(shortcutText());
        btn->setHoverColor(shortcutText());
        btn->setHeight(28.f);
        btn->setWidth(120.f);
        btn->setBg(shortcutSurface());
        btn->setHoverBg(shortcutHover());
        btn->setBorder(1.f, shortcutBorder());
        btn->onClick.add([this](Ling::Button* btn) {this->onBtnClick(btn);});
        btns.push_back(btn);

        auto border = makeChild<Ling::Node>();
        border->setHeight(1.f);
        border->setBg(shortcutBorder());
    }
